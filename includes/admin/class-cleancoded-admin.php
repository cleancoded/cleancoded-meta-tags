...
class CLEANCODED_Admin {    
    ...
    public function __construct(){
        ...
        register_activation_hook( CLEANCODED_PLUGIN_FILE, array( $this, 'on_activation' ) );
        ...
        add_filter( 'plugin_action_links_' . CLEANCODED_PLUGIN_FILE, array( $this, 'add_action_link' ) );
        ...
        add_action( 'admin_post_CLEANCODED_editor_form_submit', array( $this, 'save_meta_tags' ) );
        add_action( 'admin_post_CLEANCODED_table_bulk_submit', array( $this, 'table_bulk_actions' ) );
    ...
    public function on_update( $upgrader_object, $options ){
        ...
        if( $options['action'] == 'update' && 
            ...
            if ( in_array( CLEANCODED_PLUGIN_FILE, $options['plugins'] ) ){
    ...
    public function load_textdomain(){
        ...
        load_plugin_textdomain( 'dp-meta-tags', false, CLEANCODED_PLUGIN_DIR .'/languages' );
    ...
    public function check_version(){
        ...
        // run specific tasks
        if ( empty( get_option( 'CLEANCODED_plugin_version' ) ) ){
                       
            include_once 'class-cleancoded-migration.php';
            $migrate = new CLEANCODED_Migration();
        ...
        $plugin_info = get_plugin_data( CLEANCODED_PLUGIN_FULL_PATH );
        if ( $plugin_info['Version'] != get_option( 'CLEANCODED_plugin_version' )  ){
            
            update_option( 'CLEANCODED_plugin_version', $plugin_info['Version'] );
    ...
    public function meta_tag_pages(){
        ...
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/cleancoded-meta-tag-list.php';
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/class-cleancoded-retrieve-tags.php';

        if ( ! empty($_GET['type']) && ! empty($_GET['edit']) ){

            // editor page            
            include_once dirname( plugin_dir_path( __FILE__ ) ) . '/class-cleancoded-retrieve-info.php';
            include_once 'views/html-meta-tag-editor.php';        

        }else{

            // table page
            include_once 'class-cleancoded-retrieve-list.php';
    ...
    public function set_notices(){
        ...
        add_action( 'admin_notices', function() {
            ...
            if( $screen == 'themes.php' && ! get_user_meta( $user_id, 'CLEANCODED_ad_dismissed' ) ){
                ...
                echo '<span class="dpmt-dismiss-forever"><a href="?CLEANCODED_ad_dismissed=1"><i class="dashicons dashicons-dismiss"></i> ';
        ...
        // dismiss notice
        if ( isset($_GET['CLEANCODED_ad_dismissed']) ){
            
            $user_id = get_current_user_id();
            add_user_meta( $user_id, 'CLEANCODED_ad_dismissed', 'true', true );
    ...
    public function add_css_js(){
        ...
        wp_enqueue_style( 'CLEANCODED_admin_css', plugins_url('assets/css/admin.css', CLEANCODED_PLUGIN_FILE) );
        wp_enqueue_script( 'CLEANCODED_admin_js', plugins_url('assets/js/admin.js', CLEANCODED_PLUGIN_FILE), array('jquery') );
    ...
    public function save_meta_tags(){
        ...
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/cleancoded-meta-tag-list.php';
        include_once 'class-cleancoded-save-tags.php';       

        CLEANCODED_Save_Tags::save( $CLEANCODED_meta_tag_list, $_POST );


        // redirect to previous page
        wp_redirect( admin_url( 'options-general.php?page=dpmt-editor&type='. $_POST['CLEANCODED_type'] .'&edit='. $_POST['CLEANCODED_id'] ) );
    ...
    public function table_bulk_actions(){
        ...
        foreach( $groups as $group ){
            ...
            if ( $_POST['bulk-'. $group] != -1 ){
                ...
                CLEANCODED_Save_Tags::bulk( $_POST['bulk-'. $group], $CLEANCODED_meta_tag_list, $type, $group );
    ...
    public function add_meta_boxes(){
        ...
        foreach ($screens as $screen) {
            add_meta_box(
                'CLEANCODED_meta_tag_editor_link',
    ...
    public function always_autopilot_setting(){
        ...
        $array_to_save = maybe_unserialize( get_option('CLEANCODED_autopilot_settings') );
        ...
        update_option('CLEANCODED_autopilot_settings', $array_to_save);
