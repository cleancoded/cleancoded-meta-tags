...
class CLEANCODED_Save_Tags {
    ...
    public static function save( $list_of_meta_tags, $data ){
        ...
        // validate type
        if ( ! post_type_exists($data['CLEANCODED_type']) && ! taxonomy_exists($data['CLEANCODED_type']) ){
            return;
        }

        $type = $data['CLEANCODED_id'] == 'front' ? 'frontpage' : $data['CLEANCODED_type'];
        ...
        $url_fields = [ 
            ...
            'CLEANCODED_og_video',
            'CLEANCODED_og_url',
            'CLEANCODED_twitter_image',
            'CLEANCODED_twitter_player_stream'
        ];
            ...
            foreach ( $item['fields'] as $tag => $field ){
                ...
                switch (true){
                    ...
                    case taxonomy_exists( $type ):
                        update_term_meta( $data['CLEANCODED_id'], $key, $value );

                        break;


                    case $type == 'author':
                        update_user_meta( $data['CLEANCODED_id'], $key, $value );

                        break;


                    case $type == 'frontpage':
                        update_option( 'CLEANCODED_frontpage_'. $key, $value );

                        break;


                    case post_type_exists( $type ):
                        update_post_meta( $data['CLEANCODED_id'], $key, $value );
    ...
    public static function bulk( $action, $list_of_meta_tags, $wp_object_type, $meta_tag_group ){
        ...
        $items = CLEANCODED_Retrieve_List::get_list( $wp_object_type );
                ...
                if ( $item['id'] == 'front' ){
                    ...
                    foreach ( $fields_to_update as $field ){
                        ...
                        if ( $action == 'autopilot' ){
                            ...
                            update_option( 'CLEANCODED_frontpage_'. $field, 'auto' );

                        }elseif ( $action == 'delete' ){
            
                            delete_option( 'CLEANCODED_frontpage_'. $field );                            
