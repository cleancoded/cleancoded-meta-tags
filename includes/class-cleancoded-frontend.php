...
class CLEANCODED_Frontend {
    ...
    public function includes(){
        ...
        include_once 'cleancoded-meta-tag-list.php';
        include_once 'class-cleancoded-retrieve-info.php';
        include_once 'class-cleancoded-retrieve-tags.php';

        $this->meta_tag_list = $CLEANCODED_meta_tag_list;
    ...
    public function set_html_prefix_attribute(){
        ...
        add_filter( 'language_attributes', 'CLEANCODED_add_og_html_prefix' );

        function CLEANCODED_add_og_html_prefix( $output ){
    ...
    public function process_auto_tags( $tag_to_process ){
        ...
        if ( empty($this->page_info) ){
            $this->page_info = new CLEANCODED_Retrieve_Info( $this->page_type, $this->page_id );
        }



        // general tags - we skip keywords because it's not recommended
        if( $tag_to_process == 'CLEANCODED_general_description' ){
        ...
        // open graph
        if( $tag_to_process == 'CLEANCODED_og_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'CLEANCODED_og_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'CLEANCODED_og_type' ){
        ...
        if( $tag_to_process == 'CLEANCODED_og_audio' ){
            return $this->page_info->audio;
        }

        if( $tag_to_process == 'CLEANCODED_og_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'CLEANCODED_og_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'CLEANCODED_og_video' ){
            return $this->page_info->video;
        }

        if( $tag_to_process == 'CLEANCODED_og_url' ){
        ...
        // twitter cards
        if( $tag_to_process == 'CLEANCODED_twitter_card' ){

            if (
                ! empty( $this->page_info->video ) &&
                ( ! empty( $this->tags['CLEANCODED_twitter_player'] ) || ! empty( $this->tags['CLEANCODED_twitter_player_stream'] ) )
            ){
                return 'player';
            }
        ...
        if( $tag_to_process == 'CLEANCODED_twitter_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'CLEANCODED_twitter_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'CLEANCODED_twitter_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'CLEANCODED_twitter_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'CLEANCODED_twitter_player_stream' ){
        ...
        if( $tag_to_process == 'CLEANCODED_twitter_player_stream_content_type' ){
    ...
    public function process_tags(){
        ...
        $taginfo = new CLEANCODED_Retrieve_Tags( $this->meta_tag_list );
        ...
        $CLEANCODED_autopilot_settings = get_option('CLEANCODED_autopilot_settings');
        $autopilot_arr = maybe_unserialize($CLEANCODED_autopilot_settings);
        ...
        foreach ( $this->meta_tag_list as $group => $values ){
            ...
            foreach ($values['fields'] as $field => $info) {
                ...
                if (
                    ...
                    if ( $info['variable'] == 'CLEANCODED_custom' ){
                    // user defined extra meta tags
                        ...
                        if ( ! $this->is_og_html_attr_set && substr($info['variable'], 0, 7) == 'CLEANCODED_og' ){
...
return new CLEANCODED_Frontend();
