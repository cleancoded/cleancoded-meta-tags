...
final class CLEANCODED_Meta_Tags {
     ...
     * @var float
     */
    const CLEANCODED_REQUIRED_PHP = '5.6.3';
     ...
     * @var float
     */
    const CLEANCODED_REQUIRED_WP = '4.7';
    
    /**
     * The one and only instance of the plugin (singleton).
     *
     * @var CLEANCODED_Meta_Tags
     */
    ...
    private function check_system(){
        ...
        if (
            version_compare(PHP_VERSION, self::CLEANCODED_REQUIRED_PHP, '<') || 
            version_compare(get_bloginfo('version'), self::CLEANCODED_REQUIRED_WP, '<')
        ){

            if ( is_admin() ){          

                require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
                deactivate_plugins( CLEANCODED_PLUGIN_FILE );  
                ...
                wp_die(
                    sprintf(
                        ...
                        self::CLEANCODED_REQUIRED_PHP, 
                        self::CLEANCODED_REQUIRED_WP
                    ) . 
    ...
    private function run(){
        ...
        if ( is_admin() ){
            include_once dirname(__FILE__) . '/admin/class-cleancoded-admin.php';
        }else{
            include_once dirname(__FILE__) . '/class-cleancoded-frontend.php';
        }
